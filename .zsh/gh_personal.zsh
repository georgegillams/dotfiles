...
# alias dotfiles-save-zshrc='dotfiles && cp ~/.zshrc ./ && cp ~/.zsh/gh_* ./.zsh/ && cp ~/.zsh/typeform.zsh $MY_TF_DRIVE/.zsh/typeform.zsh && gd && sleep 5 && gaa && gcmpchore "Update ZSHRC"'
alias dotfiles-save-zshrc='dotfiles && cp ~/.zshrc ./ && cp ~/.zsh/gh_* ./.zsh/ && cp ~/.zsh/typeform.zsh $MY_PERSONAL_DRIVE/2_Areas/Work/Typeform/.zsh/typeform.zsh && gd && sleep 5 && gaa && gcmpchore "Update ZSHRC"'
...
alias gwa-start='gwa && gwa-start-transpile-watch-in-new-tab && yarn dev | spacer'
...
alias gwao='gwa && yarn dev | spacer'
...
alias geo='ge && yarn dev | spacer'
alias ge-backup='ge && 1p-personal && node scripts/backup-production-data.js --apiKey $(op item get "AWS Private REST API Key" --vault Personal --field password --cache) --backupsLocation "$MY_PERSONAL_DRIVE/1_Projects/georgegillams.co.uk/backups"'
...
alias ihho='ihh && yarn dev | spacer'
...
alias hrpo='hrp && yarn dev | spacer'
alias hrp-fix-all='hrp && run-concurrently -n test,lint,browserlist "yarn test -u" "yarn lint" "zsh -ci npx-update-browserlist-db"'
alias hrp-backup='hrp && 1p-personal && node scripts/backup-production-data.js --apiKey $(op item get "AWS Private REST API Key" --vault Personal --field password --cache) --backupsLocation "$MY_PERSONAL_DRIVE/1_Projects/Hyrox Relay Planner/backups"'
...
alias ggco='ggc && yarn dev | spacer'
...
alias boiler-deploy='boiler && rsync -avr -e "ssh -l pi" --exclude ".git" --exclude "node_modules" ./* pi@192.168.1.95:/home/pi/Documents/georgegillams/boiler-server'
alias boiler-backup='curl http://192.168.1.95:3000/logs-raw > "$MY_PERSONAL_DRIVE/2_Areas/Boiler hot water issue/Boiler restart logs/$(date).txt"'

...
alias zsh-edit-config='(dotfiles && open-code-editor .zsh) && open-code-editor ~/.zshrc && ([[ -d $HS_DRIVE ]] && open-code-editor "$HS_DRIVE/02_Areas/Apps/.zsh/hs.zsh")'
...
alias system-open-davinci='open /Applications/DaVinci\ Resolve.app || true'
alias system-open-dxo='open /Applications/DXO\ Photo\ Lab.app || true'
...
alias system-update-apps='system-update-brew-apps && system-open-cc-updater && system-open-topaz && system-open-ms-updater && && system-open-davinci && system-open-dxo && (nixt-update || true)'
...
alias system-backup-macbook='red "BEFORE RUNNING THIS, ENSURE SECRETS ARE REMOVED FROM ZSH_HISTORY.\nPRESS ANY KEY IF THIS IS ALREADY DONE" && read && open "$MY_PERSONAL_DRIVE/2_Areas/Apps" && cp ~/.zsh_history "$MY_PERSONAL_DRIVE/2_Areas/Apps/" && cp ~/.mcp.json "$MY_PERSONAL_DRIVE/2_Areas/Apps/" && cp ~/.claude/settings.json "$MY_PERSONAL_DRIVE/2_Areas/Apps/" && echo "Save a screenshot of the dock + menubar\nBackup raycast settings\nBackup widgets"'

...
function kill-processes-on-port() { for pid in \$(list-process-pids-on-port $@); do pkill -9 \$pid; done }
...
alias git-master-latest='gco master && git-fetch && git reset --hard origin/master && git pull && (git branch -D $(git-branch | grep -v "master") || true) && git submodule update'
alias git-develop-latest='gco develop && git-fetch && git reset --hard origin/develop && git pull && (git branch -D $(git-branch | grep -v "develop") || true) && git submodule update'
...
alias git-reset='git-pre-push && git-add-all-no-image-optimisation && git reset --hard HEAD'

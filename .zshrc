...
alias backpack-setup='backpack && touch native/android/local.properties && npm i && npm run build && npm run bootstrap && npm run build && cd native/ios/ && bundle install && bundle exec pod install && backpack && git submodule update'
...
alias backpack-run-android-docs='backpack && ($ANDROID_SDK_ROOT/emulator/emulator -avd NEXUS_5_API_27 || true) & (cd native && npm run android:build)'
